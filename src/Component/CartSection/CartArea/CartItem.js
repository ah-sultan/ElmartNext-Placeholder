...
const CartItem = ({ image, title, size, price, total, index }) => {
    ...
    <tr className={`wish-item ${isBg ? "no-bg":  "with-bg" }`}>
      ...
      <td className="quantity" data-title="Quantity">
        <form className="item-quantity" action="#">
          <span className="decrease qty-btn" onClick={() => setCount(count - 1)}>-</span>
          <input disabled type="number" value={count} />
          <span className="increase qty-btn" onClick={() => setCount(count + 1)}>+</span>
        </form>
      </td>
